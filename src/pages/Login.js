...
const Login = () => {
  ...
  const submitLogin = data => {
    axios.post('https://ecommerce-api-react.herokuapp.com/api/v1/users/login',data)
      .then(res => {
        ...
        localStorage.setItem('NameUser', res.data.data.user.firstName + " " + res.data.data.user.lastName)
        dispatch(setModal('Secion iniciada'))
        navigate('/')
      })
  ...
  const submitNewLogin = data => {
    ...
    .then(() => {
      dispatch(setModal('Cuenta creada, ahora inicia secion porfavor'))
      navigate('/login')
    })
    .catch(error => dispatch(setModal(error.response.data.message)))
  }
    ...
    <>
      {
        registeredUser ? (
          <div className='login already-registered'>
            ...
            <section>
              <b>Test Data</b>
              <p><i className='bx bx-envelope bx-tada' ></i> mason@gmail.com</p>
              <p><i className='bx bx-lock-alt bx-tada' ></i> mason1234</p>
            </section>
            ...
            <span>Don't have an account? <button onClick={() => setRegisteredUser(false)}>Sign up</button></span>
          </div>
        ) : (
          <div className='login'>
            ...
            <form onSubmit={handleSubmit(submitNewLogin)}>
              ...
              <div>
                ...
                <input type="password" id='new-password' {...register('password')}/>
              </div>
              <div>
            ...
            <span>Already have you an account? <button onClick={btnChange}>Log in</button></span>
          </div>
        )
      }
    </>
